        ...
        jQuery(document).ready(function($) {
            ...
            $('select[data-filter-type=select2]').not('[data-filter-enabled]').each(function () {
				...
				$(this).change(function() {
					...
					// NOTE:: global employee filter
					var employee_filters = {!! json_encode(session('employee_filter')) !!};
					$.each(employee_filters, function(index, key) {
						var url_temp = localStorage.getItem(key+'_list_url');
						if (parameter == 'employee' && url_temp != null) {
							url_start = "{{ backpack_url() }}"+"/"+key;
					        url_end = url_temp.replace(url_start, '');
					        url_end = url_end.replace('/search', '');
					        url_temp = url_start + url_end;

					        temp = addOrUpdateUriParameter(url_temp, parameter, value);
							
						 	localStorage.setItem(key+'_list_url', temp);
							// console.log(temp);
						}
						// console.log('');
					});
